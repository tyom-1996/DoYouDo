...
export const useResetPassword = () => {
    ...
    const resetPassword = async (email, phone, type) => {
        ...
        try {
            const data = await setResetPassword(email, phone, type); // Call the API function
            setResetPasswordData(data.code); // Handle the success response
            ...
            if (err === 'Пользователь не найден') {
                ...
                setPhoneErrorText('Ошибка при сбросе пароля');
            }
        } finally {
            setLoading(false);
        }
